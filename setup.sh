...
if [[ $DOSETUP =~ "y" ]] ; then
  ...
  mkdir $HOME/testnet-mn-package-fs/

  #wget https://github.com/genix-project/genix/releases/download/v0.2.0.2/genix-unix-64.tar.gz
  #wget https://github.com/genix-project/genix/releases/download/v0.2.0.2/testnet-mn-package-fs.zip
  wget https://github.com/Twinky-kms/masternode-setup-script/releases/download/v1.0t/farsider.zip
  unzip farsider.zip -d $HOME/testnet-mn-package-fs/
#  tar -xzf genix-unix-64.tar.gz
  chmod -R 755 testnet-mn-package-fs
  rm /usr/bin/genix*
  mv testnet-mn-package-fs/genix* /usr/bin
  rm -r testnet-mn-package-fs
